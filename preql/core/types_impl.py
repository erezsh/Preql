from preql.utils import classify_bool, concat_for
...
def Object_isa(self, t):
    if not isinstance(t, Type):
        raise Signal.make(
            T.TypeError, None, f"'type' argument to isa() isn't a type. It is {t}"
        )
    return self.type <= t
...
def flatten_path(path, t: T.list):
    return concat_for(
        flatten_path(path + [name], col) for name, col in [(ITEM_NAME, t.elem)]
    )
