...
from .pql_types import T, dp_inst, dp_type
from .sql import _ARRAY_SEP, mysql, sqlite
...
def _restructure_result(t: T.json_array[T.union[T.primitive, T.nulltype]], i):
    ...
    if target == mysql:
        ...
        if not isinstance(res, str):
            raise Signal.make(
                T.TypeError,
                None,
                f"json_array type expected a string separated by {_ARRAY_SEP}. Got: '{res}'",
            )
    ...
    try:
        ...
        raise Signal.make(
            T.TypeError, None, f"Error trying to convert values to type {t.elem}"
        )
...
def _extract_primitive(res, expected):
    try:
        ...
        raise Signal.make(
            T.TypeError, None, f"Expected a single {expected}. Got: '{res.value}'"
        )
...
def sql_result_to_python(res: T.bool):
    ...
    if item not in (0, 1):
        raise Signal.make(
            T.ValueError, None, f"Expected SQL to return a bool. Instead got '{item}'"
        )
    return bool(item)
...
def sql_result_to_python(res: T.int):
    ...
    if not isinstance(item, int):
        breakpoint()
        raise Signal.make(
            T.ValueError, None, f"Expected SQL to return an int. Instead got '{item}'"
        )
    return item
...
def sql_result_to_python(arr: T.list):
    ...
    if not all(len(e) == len(fields) for e in arr.value):
        raise Signal.make(
            T.TypeError, None, f"Expected 1 column. Got {len(arr.value[0])}"
        )

    if arr.type.elem <= T.struct:
        return [
            {n: _from_sql_primitive(e) for (n, _t), e in safezip(fields, tpl)}
    ...
    for row in arr.value:
        if len(row) != expected_length:
            raise Signal.make(
                T.TypeError,
                None,
                f"Expected {expected_length} columns, but got {len(row)}",
            )
        i = iter(row)
        yield {
            name: _restructure_result(col, i) for name, col in arr.type.elems.items()
        }
