...
def _compare(op, a: T.any, b: T.any):
    raise Signal.make(
        T.TypeError, op, f"Compare not implemented for {a.type} and {b.type}"
    )
...
def _prepare_to_compare(op, a, b):
    ...
    raise Signal.make(
        T.TypeError, op, f"Operator '{op}' not implemented for {a.type} and {b.type}"
    )
...
def _compare(op, a: T.primitive, b: T.primitive):
    if (
        settings.optimize
        and isinstance(a, objects.ValueInstance)
        and isinstance(b, objects.ValueInstance)
    ):
        f = {
            '=': operator.eq,
            '!=': operator.ne,
        ...
        try:
            ...
            raise Signal.make(
                T.TypeError,
                op,
                f"Operator '{op}' not implemented for {a.type} and {b.type}",
            )
...
def _compare(op, a: T.type, b: T.type):
    ...
    if op != '=':
        raise Signal.make(
            T.NotImplementedError, op, f"Cannot compare types using: {op}"
        )
...
def _contains(op, a: T.any, b: T.any):
    raise Signal.make(
        T.TypeError, op, f"Contains not implemented for {a.type} and {b.type}"
    )
...
def _compile_arith(arith, a: T.any, b: T.any):
    raise Signal.make(
        T.TypeError,
        arith.op,
        f"Operator '{arith.op}' not implemented for {a.type} and {b.type}",
    )
...
def _compile_arith(arith, a: T.table, b: T.table):
    ...
    try:
        ...
        raise Signal.make(
            T.TypeError,
            arith.op,
            f"Operation '{arith.op}' not supported for tables ({a.type}, {b.type})",
        )
...
def _compile_arith(arith, a: T.string, b: T.int):
    if arith.op != '*':
        raise Signal.make(
            T.TypeError,
            arith.op,
            f"Operator '{arith.op}' not supported between string and integer.",
        )
...
def _compile_arith(arith, a: T.number, b: T.number):
    ...
    try:
        ...
        raise Signal.make(
            T.TypeError,
            arith,
            f"Operator {arith.op} not supported between types '{a.type}' and '{b.type}'",
        )
...
def _compile_arith(arith, a: T.string, b: T.string):
    ...
    if arith.op != '+':
        raise Signal.make(
            T.TypeError, arith.op, f"Operator '{arith.op}' not supported for strings."
        )
