from lark import ParseError, Token, UnexpectedCharacters, UnexpectedToken
...
def eval_autocomplete(t: ast.Try, go_inside):
    ...
    scope = (
        {t.catch_name: Signal(catch_type, [], '<unknown exception>')}
        if t.catch_name
        else {}
    )
...
class AcState(ThreadState):
    ...
    def get_all_vars_with_rank(self):
        all_vars = {
            k: (10000, v) for k, v in self.get_var('__builtins__').namespace.items()
        }
