...
from preql.utils import SafeDict, find_duplicate, listgen, method, re_split, safezip

from . import pql_ast as ast
from . import pql_objects as objects
from . import sql
from .casts import cast
from .compile_binops import compare, compile_arith, contains
...
from .pql_objects import AbsInstance, make_instance, projected
from .pql_types import ITEM_NAME, Id, T, Type
from .state import (
    AccessLevels,
    get_access_level,
    get_db,
...
from .types_impl import flatten_type, kernel_type, pql_repr
...
def cast_to_instance(x):
    ...
    if not isinstance(inst, AbsInstance):
        ...
        raise Signal.make(
            T.TypeError, None, f"Could not compile {pql_repr(inst.type, inst)}"
        )
    ...
    for f in fields:
        ...
        try:
            ...
            if e.type <= T.TypeError:
                raise e.replace(
                    message=f"Cannot use object of type '{evaluate( f.value).type}' in projection."
                )
            raise
...
def _expand_ellipsis(obj, fields):
    ...
    for f in fields:
        ...
        if isinstance(f.value, ast.Ellipsis):
            ...
            t = obj.type
            assert (
                t <= T.table or t <= T.struct
            )  # some_table{ ... } or some_table{ some_struct_item {...} }

            for n in f.value.exclude:
                if isinstance(n, ast.Marker):
                    raise AutocompleteSuggestions(
                        {
                            k: (0, v)
                ...
                if n in direct_names:
                    raise Signal.make(
                        T.NameError,
                        n,
                        f"Field to exclude '{n}' is explicitely included in projection",
                    )
            ...
            if f.value.from_struct:
                ...
                with use_scope(obj.all_attrs()):
                    ...
                    if not s.type <= T.struct:
                        raise Signal.make(
                            T.TypeError, s, f"Cannot inline objects of type {s.type}"
                        )
            ...
            try:
                remaining_items = list(
                    _exclude_items(items, set(f.value.exclude), direct_names)
                )
            except ValueError as e:
                fte = set(e.args[0])
                raise Signal.make(
                    T.NameError, obj, f"Fields to exclude '{fte}' not found"
                )
            ...
            for name, value in remaining_items:
                ...
                yield ast.NamedField(name, value, user_defined=False).set_text_ref(
                    f.text_ref
                )
        else:
            yield f
...
def compile_to_inst(proj: ast.Projection):
    ...
    if not table.type <= t:
        raise Signal.make(
            T.TypeError, proj, f"Cannot project objects of type {table.type}"
        )
    ...
    dup = find_duplicate(
        [f for f in list(proj.fields) + list(proj.agg_fields) if f.name],
        key=lambda f: f.name,
    )
    if dup:
        raise Signal.make(
            T.TypeError, dup, f"Field '{dup.name}' was already used in this projection"
        )

    attrs = (
        table.all_attrs()
    )  # TODO separate here between columns and methods? (not it's done in _process_fields)
    ...
    for name, f in fields:
        if not f.type <= T.union[T.primitive, T.struct, T.json, T.nulltype, T.unknown]:
            raise Signal.make(
                T.TypeError, proj, f"Cannot project values of type: {f.type}"
            )
    ...
    # Make code
    flat_codes = [code for _, inst in all_fields for code in inst.flatten_code()]
    ...
    if not sql_fields:
        raise Signal.make(
            T.TypeError, proj, "No column provided for projection (empty projection)"
        )

    # Make Instance
    new_table = objects.TableInstance.make(
        sql.null, new_table_type, [table] + [inst for _, inst in all_fields]
    )
    ...
    code = sql.Select(
        new_table_type, table.code, sql_fields, group_by=groupby, limit=limit
    )
...
def compile_to_inst(order: ast.Order):
    ...
    for f in fields:
        if not f.type <= T.primitive:
            ...
            raise Signal.make(
                T.TypeError, order, f"Arguments to 'order' must be primitive"
            )
...
def compile_to_inst(lst: ast.List_):
    ...
    if len(types) > 1:
        raise Signal.make(
            T.TypeError, lst, f"List members must be of the same type. Got {types}"
        )
    ...
    if elem_type <= T.struct:
        ...
        if elem_type <= (T.projected | T.aggregated):
            raise Signal.make(
                T.TypeError,
                lst,
                "Cannot create lists of projections or aggregations (%s)" % elem_type,
            )
        if not (elem_type <= T.union[T.primitive, T.nulltype]):
            raise Signal.make(
                T.TypeError, lst, "Cannot create lists of type %s" % elem_type
            )
...
def compile_to_inst(res: ast.ResolveParameters):
    ...
    subqueries = {
        k: _resolve_sql_parameters(v, subqueries=sq2) for k, v in obj.subqueries.items()
    }
...
def compile_to_inst(rps: ast.ParameterizedSqlCode):
    ...
    for m, t in tokens:
        if m:
            ...
            if t == '$self':
                if self_table is None:
                    raise Signal.make(
                        T.TypeError,
                        rps,
                        f"$self is only available for queries that return a table",
                    )
                inst = self_table
            else:
                obj = get_var(t[1:])
            ...
            new_code += _resolve_sql_parameters(
                inst.code, wrap=bool(new_code), subqueries=subqueries
            ).code
...
def compile_to_inst(s: ast.Slice):
    ...
    if obj.type <= T.string:
        code = sql.StringSlice(
            obj.code, sql.add_one(start.code), stop and sql.add_one(stop.code)
        )
    else:
...
def compile_to_inst(sel: ast.Selection):
    ...
    if table.type <= T.string:
        ...
        table = table.replace(
            type=T.string
        )  # XXX why get rid of projected here? because it's a table operation node?
        slice = ast.Slice(
            table, ast.Range(index, ast.BinOp('+', [index, ast.Const(T.int, 1)]))
        ...
        for i, c in enumerate(conds):
            if not c.type <= T.bool:
                raise Signal.make(
                    T.TypeError,
                    sel.conds[i],
                    f"Selection expected boolean, got {c.type}",
                )
...
def compile_to_inst(attr: ast.Attr):
    ...
    if not attr.expr:
        raise Signal.make(
            T.NotImplementedError, attr, "Implicit attribute syntax not supported"
        )
...
def _apply_type_generics(gen_type, type_names):
    ...
    if not type_objs:
        raise Signal.make(
            T.TypeError, None, f"Generics expression expected a type, got nothing."
        )
    for o in type_objs:
        if not isinstance(o, Type):
            ...
            raise Signal.make(
                T.TypeError, None, f"Generics expression expected a type, got '{o}'."
            )
...
def compile_to_inst(marker: ast.Marker):
    all_vars = (
        context.state.get_all_vars_with_rank()
    )  # Uses overridden version of AcState
...
def compile_to_inst(range: ast.Range):
    ...
    if range.stop:
        ...
        raise Signal.make(
            T.NotImplementedError, range, f"{target} doesn't support infinite series!"
        )
    ...
    if target == sql.bigquery:
        ...
        code = sql.RawSql(
            type_, f'SELECT * FROM UNNEST(GENERATE_ARRAY({start}, {stop-1})) as item'
        )
